# -*- coding: utf-8 -*-
"""
Created on Thu Feb 21 12:14:08 2019
...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done, regulator=1.0):
        ...
        if self.experience_count > self.buffer_size:
            ...
            if temp.priority == self.priorities_max:
                d = self.data(0, self.memory_data[index].probability, 
                              self.memory_data[index].weight, index)
            ...
            if self.compute_weights:
                if temp.weight == self.weights_max:
                    d = self.data(self.memory_data[index].priority, self.memory_data[index].probability, 
                                  0, index)
                    self.memory_data[index] = d
                    self.weights_max = max(self.memory_data.items(), key=operator.itemgetter(1))[1].weight
                    
                    
        priority = self.priorities_max*regulator
    ...
    def sample(self):
        ...
        actions = np.vstack([e.action for e in experiences if e is not None]).astype(int)
        rewards = np.vstack([e.reward for e in experiences if e is not None])
        next_states = np.vstack([e.next_state for e in experiences if e is not None])
        dones = np.vstack([e.done for e in experiences if e is not None]).astype(int)
